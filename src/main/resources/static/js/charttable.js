...
function getData()
{
    ...
    $.ajax({
        type:"GET",
        url:"http://fat.fatmou.se/api/data",
        data: {device_id: getDEVICE_ID(), report_id: getREPORT_ID(), page:0, size:10},
        dataType:"json",
        async: false,
        success:function(data){
            ...
            if (dataNum == 0)
                return;
            var field_type = []
            for (var tmpkey in data.data[0].payload)
            {
                if (data.data[0].payload[tmpkey] != '' && !isNaN(data.data[0].payload[tmpkey]))
                {
                    field_type.push(tmpkey);
                    lineNum += 1;
                }
            }
            for(i=0;i<dataNum;i++)
            ...
            {
                var tempArr = [];
                var tmpkey = field_type[i];
                for(j=0;j<dataNum;j++)
                {
                    tempArr.push(data.data[j].payload[tmpkey]);
                }
                dataset.push(tempArr);
                lineNames.push(tmpkey);
            }
        }
    });
    //
